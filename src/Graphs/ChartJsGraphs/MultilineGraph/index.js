...
const lineColors = ["#4D80E0", "#4D80E0", "#4666A2", "#4666A2"];
...
const MultilineGraph = ({
  ...
  const labels = data.data.map(
    (ele) => ele.month.slice(0, 3) + "'" + ele.year.slice(-2)
  );
      ...
      tooltip: {
        ...
        external: function (context) {
          ...
          if (canvas) {
            ...
            const tooltipConHeight = tooltipRef?.current?.offsetHeight || 90;
                  ...
                  return {
                    ...
                    change:
                      data?.data[hoveredIndex][
                        `monthly_change${ele?.datasetIndex}`
                      ],
                    direction:
                      data?.data[hoveredIndex][
                        `monthly_change${ele?.datasetIndex}`
  ...
  useEffect(() => {
    const multiLineDataset = data.items.map((ele, idx) => {
      return {
        label: ele,
        data: data.data.map((ele) => ele["value" + idx]),
        fill: false,
        segment: {
          borderColor: (ctx) => {
  ...
  return (
    <LineGraph>
      ...
      <TooltipWrapper
        ...
        hasSingleData={tooltipData?.body?.data.length === 1}
      >
        {tooltipVisible && tooltipData && (
          <NewTooltip
